  ...
  return (
    <div style={{ background: '#f8fafc', border: '0.5px solid #eef1f4', borderRadius: 14, padding: '.85rem 1rem' }} className="flex gap-4">
      <div className="flex flex-col justify-center shrink-0">
        <p className="font-semibold uppercase tracking-widest text-[#94a3b8] mb-1" style={{ fontSize: 10 }}>千葉南 avg score</p>
...
export default function ChibaSouthPage() {
  ...
  useEffect(() => {
    ...
    async function fetchCacheTime() {
      try {
        ...
        const db = getDb()
        const snap = await getDoc(doc(db, 'forecastCache', `kamogawa_${toDateStr(today)}`))
        if (snap.exists()) {
    ...
    if (scheduleHour !== null) {
      fetch(`/api/daily-comment?target=today&hour=${padHour(scheduleHour)}&areaLabel=千葉南&spotName=鴨川`)
  ...
  useEffect(() => {
    ...
    const activeSpots = SPOTS.filter(s => s.isActive && s.area === 'chiba-south')
    setLoading(true)
