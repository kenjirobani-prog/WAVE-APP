  ...
  useEffect(() => {
    ...
    async function loadWeeklyFromCache() {
      ...
      try {
        ...
        const db = getDb()
        const snap = await getDoc(doc(db, 'weeklyComments', 'shonan'))
        if (snap.exists()) {
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#f0f9ff]">
      ...
      <main className="flex-1 overflow-auto pb-4">
        {tab === 'weekly' ? (
          weeklyLoading ? (
            <div className="p-4"><WeeklyListSkeleton /></div>
          ) : (
          ...
          <>
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 gap-4">
                <p className="text-sm text-center px-4" style={{ color: 'var(--ink-500)' }}>{error}</p>
                <button
